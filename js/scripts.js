...
let pokemonRepository = (function () {
  ...
  function loadDetails(pokemon) {
    ...
    if (!pokemon.detailsUrl) {
      return Promise.reject(
        new Error("No details URL provided for the Pokémon.")
      )
    }

    let detailsPromise = fetch(pokemon.detailsUrl).then((response) =>
      response.json()
    )
  ...
  function parseEvolutionaryTree(evolutionaryTreeJson) {
    ...
    function traverse(evolutionaryStep) {
      ...
      if (
        evolutionaryStep.evolves_to &&
        evolutionaryStep.evolves_to.length > 0
      ) {
  ...
  function getEvolutionaryTreeUrl(pokemon) {
    if (!pokemon.name) {
      return Promise.reject(
        new Error("Pokemon object does not have a name property")
      )
    }
    ...
    return fetch(evolutionaryTreeUrl)
      ...
      .catch((e) => {
        console.error(
          `Error fetching evolutionary tree URL for ${pokemonName}:`,
          e
        )
  ...
  function deriveEvolutionaryTree(pokemon) {
    ...
    return getEvolutionaryTreeUrl(pokemon)
      ...
      .catch((error) => {
        console.error(
          `Error in fetching evolutionary tree for ${pokemon.name}:`,
          error
        )
  ...
  function showEvolutionaryTreeDetails(pokemon) {
    ...
    let detailPromises = evolutionaryTree.map((name) => {
      ...
      return fetchAndAddPokemon(name).then((newPokemon) =>
        loadDetails(newPokemon)
      )
    })
  ...
  function revealPokemonDetails(pokemon) {
    ...
    const cardBackgroundImage =
      detailedPokemon[pokemon.name].imgUrls.other["official-artwork"][
        "front_default"
      ]
    ...
    if (
      detailedPokemon[pokemon.name].imgUrls.other["dream_world"][
        "front_default"
      ]
    ) {
      pokeImage =
      ...
      pokeImage =
        detailedPokemon[pokemon.name].imgUrls.other["home"]["front_default"]
    }
    if (
      pokeName &&
      pokeHeightValue &&
      pokeWeightValue &&
      pokeTypesValue &&
      detailedPokemon[pokemon.name]
    ) {
      pokeName.innerText = `Name: ${
        pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1)
      }`
      pokeHeightValue.innerText = `${
        detailedPokemon[pokemon.name].height
      } decimeters`
      pokeWeightValue.innerText = `${
        detailedPokemon[pokemon.name].weight
      } hectograms`
      ...
      // assign side images
      showEvolutionaryTreeDetails(pokemon).then(function () {
        let sideImageOrder = determineSideImageOrder(
          pokemon,
          detailedPokemon[pokemon.name].forms
        )
        let leftImageUrl = detailedPokemon[sideImageOrder.leftImage]
          ? detailedPokemon[sideImageOrder.leftImage].imgUrls["front_default"]
          : null
        let rightImageUrl = detailedPokemon[sideImageOrder.rightImage]
          ? detailedPokemon[sideImageOrder.rightImage].imgUrls["front_default"]
          : null
        ...
        if (rightImageUrl != null) {
          ...
          rightImageElement.setAttribute(
            "alt",
            "Next pokemon evolutionary form"
          )
  ...
  function showDetails(pokemon) {
    ...
    loadDetails(pokemon).then(() => revealPokemonDetails(pokemon))
  }
